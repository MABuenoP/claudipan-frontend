...
import { Star, ShoppingBag, Eye, Sparkles, Tag } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView }) => {
  ...
  const nombre = product.nombre || product.name || 'Producto Artesanal';
  ...
  const enOferta = product.enOferta || (precioOferta !== undefined && precioOferta > 0 && precioOferta < precio);
  const imagenUrl = product.imagenUrl || product.image || 'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=500&auto=format&fit=crop';
  const stock = product.stock !== undefined ? product.stock : 10;
  const categoria = product.categoriaNombre || product.category || 'Panadería';

  const precioFinal = enOferta && precioOferta ? precioOferta : precio;
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-amber-100/50 dark:bg-stone-950">
        ...
        {/* Overlay gradient */}
        <div className="absolute inset-0 bg-gradient-to-t from-stone-950/70 dark:from-stone-950 via-transparent to-transparent opacity-80" />

        {/* Badges */}
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10 max-w-[85%]">
          {enOferta ? (
            <span className="inline-flex items-center gap-1 bg-red-600 text-white font-extrabold text-[10px] uppercase px-2.5 py-1 rounded-full shadow-md animate-pulse">
          ...
          {stock <= 5 && stock > 0 && (
            <span className="bg-amber-600 text-white font-bold text-[10px] uppercase px-2 py-0.5 rounded-full shadow-sm">
              Últimas {stock} unid.
            </span>
          )}
        </div>
        ...
        {onQuickView && (
          <button
            ...
            className={`absolute top-3 right-3 p-2 bg-white/90 dark:bg-stone-950/80 backdrop-blur-md hover:bg-amber-100 dark:hover:bg-stone-900 text-stone-800 dark:text-stone-200 hover:text-amber-600 dark:hover:text-amber-400 rounded-xl border border-amber-200/80 dark:border-stone-700 transition-all duration-200 ${
      ...
      <div className="p-5 flex flex-col flex-1 justify-between gap-3">
        <div>
          <div className="flex items-center justify-between text-xs text-stone-500 dark:text-stone-400 mb-1">
            <span className="font-semibold text-amber-700 dark:text-amber-400 truncate max-w-[150px]">{categoria}</span>
            <span className="text-[11px] font-bold text-stone-400">Stock: {stock}</span>
          </div>
          ...
          <div>
            {enOferta && precioOferta ? (
              <div className="flex flex-col">
                <span className="text-[11px] text-stone-400 line-through font-bold">
                  {formatCurrency(precio)}
                </span>
                <span className="text-lg font-heading font-extrabold text-red-600 dark:text-red-400">
